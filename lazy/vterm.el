...
(defvar vterm-esc-map (make-keymap))
(define-key vterm-mode-map [remap self-insert-command] 'vterm--self-insert)
(define-key vterm-mode-map "\e" vterm-esc-map)
(define-key vterm-mode-map "\e\e\e" '(lambda()(interactive) (vterm-send-key "\e" )))
...
(defun vterm-send-key (key &optional shift meta ctrl)
  ...
  (when vterm--term
    (let* ((inhibit-redisplay t)
           (inhibit-read-only t)
           (key key))
      (when (and shift (not meta) (not cctrl))
        (setq key (upcase key)))
      (vterm--update vterm--term key shift meta ctrl))))
